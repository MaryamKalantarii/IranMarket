...
def show_similar_app(product):
    ...
    similar_app = Appliances.objects.filter(status=True, category_Appliances__in=app_categories).order_by('-created_date')[:6]
...
def show_similar_supermarket(product):
    ...
    similar_supermarket = Supermarket.objects.filter(status=True, category_Supermarket__in=supermarket_categories).order_by('-created_date')[:6]
...
def show_similar_child(product):
    ...
    similar_child = Child_and_baby.objects.filter(status=True, category_Child_and_baby__in=child_categories).order_by('-created_date')[:6]
